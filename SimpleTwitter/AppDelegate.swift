...
//        TwitterSessionManager.sharedInstance.fetchCurrentAccount(completion: { (user: User) in
//            print("succeess with user \(user.name)")
//            }, failure: { (erro: Error) in
//                print("failure to verify current account")
//        })
        
        if let currentUser = User.currentUser {
            print("User already logged in: \(currentUser.name)")
            window?.rootViewController = LoginViewController.getRootVCAfterLogin()
        }
        else {
    ...
    func application(_ app: UIApplication, open url: URL, options: [UIApplicationOpenURLOptionsKey : Any] = [:]) -> Bool {
        ...
        TwitterSessionManager.sharedInstance.completeLogin(with: url)
